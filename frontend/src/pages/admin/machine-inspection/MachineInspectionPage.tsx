...
const getMachineCode = (item: ApiMachineInspection) => {
...
const getTechnicianName = (item: ApiMachineInspection) => {
...
export default function MachineInspectionPage() {
  ...
  const [addForm, setAddForm] = useState<InspectionForm>(createEmptyForm());
  ...
  const startEdit = useCallback((r: ApiMachineInspection) => {
    setEditId(r._id);
    setShowAdd(false);
    setEditForm(inspectionToForm(r));
  }, []);
      ...
      {activeForm && (
        <div className={styles.formCard}>
          <h3 className={styles.formCardTitle}>
            {isEditing ? <><Pencil size={18} />تعديل فحص آلة</> : <><Plus size={18} />فحص آلة جديد</>}
          </h3>
