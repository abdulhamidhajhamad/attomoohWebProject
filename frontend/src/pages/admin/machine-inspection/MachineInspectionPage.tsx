...
import { Search as SearchIcon, Plus, Search, Trash2, Pencil, X, Check, FileText } from 'lucide-react';
...
export default function MachineInspectionPage() {
  const { items, loading, error, fetchAll, createItem, updateItem, deleteItem, clearError } = useMachineInspectionStore();
  ...
  const startEdit = useCallback((r: ApiMachineInspection) => { setEditId(r._id); setEditForm({ machineName: r.machineName, machineDetails: r.machineDetails, technicianName: r.technicianName, date: r.date, technicianReport: r.technicianReport }); }, []);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div><h1 className={styles.pageTitle}><SearchIcon size={24} />إدارة فحص الآلات</h1><p className={styles.pageSubtitle}>فحص الآلات وتقارير الفنيين</p></div>
          ...
          <tbody>
            {showAdd && (
              <tr className={styles.addFormRow}>
                ...
                <td><input className={styles.formInput} placeholder="الفني" value={addForm.technicianName} onChange={e => setAddForm({...addForm, technicianName: e.target.value})} /></td>
                <td><input className={styles.formInput} type="date" value={addForm.date} onChange={e => setAddForm({...addForm, date: e.target.value})} /></td>
                <td>—</td><td>—</td>
            ...
            {items.map(r => editId === r._id ? (
              <tr key={r._id} className={styles.addFormRow}>
                ...
                <td><input className={styles.formInput} value={editForm.technicianName} onChange={e => setEditForm({...editForm, technicianName: e.target.value})} /></td>
                <td><input className={styles.formInput} type="date" value={editForm.date} onChange={e => setEditForm({...editForm, date: e.target.value})} /></td>
                <td>—</td><td>—</td>
