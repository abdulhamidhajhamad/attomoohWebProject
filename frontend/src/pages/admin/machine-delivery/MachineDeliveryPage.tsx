...
import { Upload, Plus, Search, Trash2, Pencil, X, Check, FileText } from 'lucide-react';
...
interface AddForm { machineName: string; machineDetails: string; customerName: string; deliveryDate: string; notes: string; }
const EMPTY: AddForm = { machineName: '', machineDetails: '', customerName: '', deliveryDate: '', notes: '' };
...
export default function MachineDeliveryPage() {
  ...
  const handleAdd = useCallback(async () => {
    if (!addForm.machineName.trim()) return; setSaving(true);
  ...
  const startEdit = useCallback((r: ApiMachineDelivery) => { setEditId(r._id); setEditForm({ machineName: r.machineName, machineDetails: r.machineDetails, customerName: r.customerName, deliveryDate: r.deliveryDate, notes: r.notes }); }, []);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div><h1 className={styles.pageTitle}><Upload size={24} />إدارة تسليم الآلات</h1><p className={styles.pageSubtitle}>تسليم الآلات للزبائن</p></div>
      ...
      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          <thead><tr><th>اسم الآلة</th><th>تفاصيل</th><th>الزبون</th><th>تاريخ التسليم</th><th>ملاحظات</th><th>إجراءات</th></tr></thead>
          <tbody>
            {showAdd && (
              <tr className={styles.addFormRow}>
                <td><input className={styles.formInput} placeholder="اسم الآلة *" value={addForm.machineName} onChange={e => setAddForm({...addForm, machineName: e.target.value})} /></td>
                <td><input className={styles.formInput} placeholder="تفاصيل" value={addForm.machineDetails} onChange={e => setAddForm({...addForm, machineDetails: e.target.value})} /></td>
                <td><input className={styles.formInput} placeholder="الزبون" value={addForm.customerName} onChange={e => setAddForm({...addForm, customerName: e.target.value})} /></td>
                <td><input className={styles.formInput} type="date" value={addForm.deliveryDate} onChange={e => setAddForm({...addForm, deliveryDate: e.target.value})} /></td>
            ...
            {items.map(r => editId === r._id ? (
              <tr key={r._id} className={styles.addFormRow}>
                <td><input className={styles.formInput} value={editForm.machineName} onChange={e => setEditForm({...editForm, machineName: e.target.value})} /></td>
                ...
                <td><input className={styles.formInput} type="date" value={editForm.deliveryDate} onChange={e => setEditForm({...editForm, deliveryDate: e.target.value})} /></td>
              ...
              <tr key={r._id}>
                ...
                <td>{r.deliveryDate ? new Date(r.deliveryDate).toLocaleDateString('ar') : '—'}</td>
                <td>{r.notes || '—'}</td>
