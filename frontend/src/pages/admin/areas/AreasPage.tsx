...
interface AddForm { city: string; village: string; phonePrefix: string; }
const EMPTY: AddForm = { city: '', village: '', phonePrefix: '' };
...
export default function AreasPage() {
  ...
  const startEdit = useCallback((r: ApiArea) => { setEditId(r._id); setEditForm({ city: r.city, village: r.village, phonePrefix: r.phonePrefix }); }, []);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          <thead><tr><th>الرمز</th><th>المدينة</th><th>القرية</th><th>رمز الهاتف</th><th>الحالة</th><th>إجراءات</th></tr></thead>
          <tbody>
            {showAdd && (
              <tr className={styles.addFormRow}>
                <td><span className={styles.customId}>تلقائي</span></td>
                <td><input className={styles.formInput} placeholder="المدينة *" value={addForm.city} onChange={e => setAddForm({...addForm, city: e.target.value})} /></td>
                <td><input className={styles.formInput} placeholder="القرية" value={addForm.village} onChange={e => setAddForm({...addForm, village: e.target.value})} /></td>
            ...
            {items.map(r => editId === r._id ? (
              <tr key={r._id} className={styles.addFormRow}>
                ...
                <td><input className={styles.formInput} value={editForm.city} onChange={e => setEditForm({...editForm, city: e.target.value})} /></td>
                <td><input className={styles.formInput} value={editForm.village} onChange={e => setEditForm({...editForm, village: e.target.value})} /></td>
