...
import { MachineReception, MachineReceptionDocument } from '../schemas/machine-reception.schema.js';

@Injectable()
export class MachineReceptionRepository {
  constructor(@InjectModel(MachineReception.name) private readonly model: Model<MachineReceptionDocument>) {}

  async create(data: Partial<MachineReception>): Promise<MachineReceptionDocument> { return new this.model(data).save(); }
  ...
  async findAll(filter: Record<string, unknown> = {}): Promise<MachineReceptionDocument[]> {
  ...
  async deleteById(id: Types.ObjectId): Promise<MachineReceptionDocument | null> {
