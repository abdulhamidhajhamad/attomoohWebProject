...
export class TransportService {
  ...
  async create(dto: CreateTransportDto): Promise<TransportDocument> {
    return this.repo.create({
      ...
      logisticFee: dto.logisticFee ?? 0, companyFee: dto.companyFee ?? 0,
    });
  }
  ...
  async startWork(id: Types.ObjectId): Promise<TransportDocument> { const d = await this.findById(id); d.timeLogs.push({ action: 'start', timestamp: new Date(), pauseReason: '' } as any); return d.save(); }
  async pauseWork(id: Types.ObjectId, reason: string): Promise<TransportDocument> { const d = await this.findById(id); d.timeLogs.push({ action: 'pause', timestamp: new Date(), pauseReason: reason ?? '' } as any); return d.save(); }
  async resumeWork(id: Types.ObjectId): Promise<TransportDocument> { const d = await this.findById(id); d.timeLogs.push({ action: 'resume', timestamp: new Date(), pauseReason: '' } as any); return d.save(); }
  async finishWork(id: Types.ObjectId): Promise<TransportDocument> {
    const d = await this.findById(id);
    d.timeLogs.push({ action: 'finish', timestamp: new Date(), pauseReason: '' } as any);
  ...
  private calcDuration(logs: Array<{ action: string; timestamp: Date }>): number {
    ...
    for (const l of logs) { if (l.action === 'start' || l.action === 'resume') start = new Date(l.timestamp); else if ((l.action === 'pause' || l.action === 'finish') && start) { total += new Date(l.timestamp).getTime() - start.getTime(); start = null; } }
    return total;
  }
}
