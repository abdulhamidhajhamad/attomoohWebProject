...
export class ToolsMgmtService {
  ...
  async create(dto: CreateToolDto): Promise<ToolDocument> {
    const customId =
      dto.customId || (await this.idGen.generateId(IdPrefix.TOOL));
    return this.repo.create({
      ...
      responsibleTechnician: dto.responsibleTechnician
        ? new Types.ObjectId(dto.responsibleTechnician)
        : undefined,
  ...
  async findById(id: Types.ObjectId): Promise<ToolDocument> {
  ...
  async update(id: Types.ObjectId, dto: UpdateToolDto): Promise<ToolDocument> {
    ...
    if (dto.responsibleTechnicianName !== undefined)
      data.responsibleTechnicianName =
        dto.responsibleTechnicianName?.trim() ?? '';
    if (dto.responsibleTechnician !== undefined)
      data.responsibleTechnician = dto.responsibleTechnician
        ? new Types.ObjectId(dto.responsibleTechnician)
        : null;
